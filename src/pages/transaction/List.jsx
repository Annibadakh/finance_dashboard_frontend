  ...
  const columns = [
    {
      ...
      accessor: "date",
      sortable: true,
      render: (row) => format(row.date, "dd MMM yyyy"),
    },
    {
      header: "Description",
    ...
    { header: "Category", accessor: "category", sortable: false },
    {
      ...
      render: (row) => (
        <span
          className={`font-bold ${row.type === "income" ? "text-emerald-600 dark:text-emerald-400" : "text-gray-900 dark:text-white"}`}
        >
    ...
    },
    {
      ...
      render: (row) => (
        <span
          className={`px-2.5 py-1 rounded-full text-xs font-medium capitalize ${row.type === "income" ? "bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400" : "bg-rose-100 text-rose-700 dark:bg-rose-900/30 dark:text-rose-400"}`}
        >
          {row.type}
        </span>
      ),
          ...
          {isAdmin && (
            <>
              <Button
                ...
                iconLeft={FiEdit2}
                onClick={() =>
                  navigate(`/dashboard/transactions/${row.id}/edit`)
                }
              />
              <Button
                variant="danger"
