  ...
  return (
    <div className="bg-white dark:bg-gray-900 p-6 sm:p-8 rounded-3xl border border-gray-200 dark:border-gray-800 shadow-sm">
      <div className="flex flex-col items-center text-center pb-8 border-b border-gray-100 dark:border-gray-800 mb-8">
        ...
        <h1
          className={`text-4xl font-bold tracking-tight ${data.type === "income" ? "text-emerald-600 dark:text-emerald-400" : "text-gray-900 dark:text-white"}`}
        >
          {data.type === "income" ? "+" : "-"}₹{data.amount.toFixed(2)}
        </h1>
        <span
          className={`mt-4 px-3 py-1 rounded-full text-xs font-semibold uppercase ${data.type === "income" ? "bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400" : "bg-rose-100 text-rose-700 dark:bg-rose-900/30 dark:text-rose-400"}`}
        >
          {data.type}
        </span>
      </div>
